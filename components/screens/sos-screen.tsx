...
export function SOSScreen({ onClose }: SOSScreenProps) {
  ...
  useEffect(() => {
    ...
    // Decrement the visible countdown every second. We avoid calling
    // `triggerSOS()` directly inside the state-updater to prevent React
    // from running a parent state update while rendering a child (which
    // can cause the "Cannot update a component while rendering a different component" error).
    // Instead, a separate effect below watches `countdownSeconds` and
    // performs the SOS trigger when it reaches 0.
  ...
  // Watch for countdown reaching 0 and trigger SOS from an effect
  useEffect(() => {
    if (!countdownActive) return;
    if (countdownSeconds !== 0) return;

    // Perform side-effects (call context and update other state) inside an effect
    // so that React does not consider these updates as occurring during render.
    triggerSOS();
