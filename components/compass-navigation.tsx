  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 w-full">
      ...
      <div className="text-center">
        {/* Normalize bearing for textual display (0-360) while keeping transform using raw value */}
        {(() => {
          const normalized = Math.round(((bearing % 360) + 360) % 360);
          return (
            <>
              <div className="text-5xl font-bold text-safe">{normalized}°</div>
              <div className="text-2xl font-bold text-foreground mt-2">{getTextDirection(normalized)}</div>
            </>
          );
        })()}
      </div>
